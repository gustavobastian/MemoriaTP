...
\end{figure}
\pagebreak
\item Tabla Paciente (\textit{Patient}): tabla que contiene la información de los pacientes. En esta tabla el id no se incrementa automáticamente (ya es asignado manualmente por el administrador). Ademas, los campos \textit{bedId}, \textit{notesTableId} y \textit{userTableId} contienen claves foraneas que identifican un elemento de la tabla
...
Luego de eso genera un \textit{token} que espira luego de un cierto tiempo dado por una variable de entorno (JWT_EXP_TIM) y se responde al front end.
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=.09]{./Figures/app/capturaQR.jpg}
